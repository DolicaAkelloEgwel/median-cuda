extern "C"{
    ...
    __global__ void two_dim_remove_outliers(float* data_array, const float* padded_array, const int X, const int Y, const int filter_height, const int filter_width, const float diff)
    {
        unsigned int id_x = blockIdx.y*blockDim.y + threadIdx.y;
        unsigned int id_y = blockIdx.z*blockDim.z + threadIdx.z;
        ...
        if (abs(data_array[index] - median) >= diff)
