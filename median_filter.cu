extern "C"{
    ...
    __device__ void print_id_info(const int id_x, const int id_y)
    {
        if (id_x == id_y && id_x == 0)
        {
            printf("Unsorted neighbour array of %d-%d:\n", id_x, id_y);
        }
    }
    __device__ float print_neighbour_elements(const float* padded_array, const int index_offset, const int padded_img_width, const int id_x, const int id_y, const int filter_size)
    {
